...
//int main()
//{
//	//大概结构是MAX = MAXCHILD X MAXCHILD个文件，二维
//	//最多MAXCHILD个用户，每个用户最多MAXCHILD个文件
//	int i, j, choice1;
//	char choice[50];  /*choice operation:dir,create,delete,open,delete,modify,read,write*/
//	int choiceend = 1;  /*该条指令是否结束*/
...
//				{
//					//ufd[j]代表用户，fcount[j]对应用户的文件数
//					strcpy(str, "c:\\osfile\\mfd");
//					strcat(str, ufd_lp.ufdname);   //strcat字符串连接函数
//					ufd[j] = (OSUFD*)malloc(sizeof(OSUFD));
//					strcpy(ufd[j]->ufdname, strupr(ufd_lp.ufdname));
//					fp_ufd = fopen(str, "rb"); //读取
//					fcount[j] = 0; //这位用户的文件数
...
//			count = count + fcount[i]; //所有用户的文件数目
//		}
//		printf("\n %3d dir(s),%5d file(s)\n", ucount, count);
//	}
//}
...
//	else if (strcmp(strupr(dname), "CD..") == 0)  strcpy(ltrim(rtrim(dirname)), ""); //发挥上层目录->''
